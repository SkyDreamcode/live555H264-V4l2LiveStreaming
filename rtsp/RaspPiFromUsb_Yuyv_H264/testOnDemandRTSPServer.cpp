...
Boolean reuseFirstSource = true;//xuyuanwen
...
static void *recv_from_usb_thread(void *arg)
{
	...
	printf("thread recv_from_usb_thread, pipe_fd = %d\n", pipe_fd);
	...
	while(1)
	{
		...
		if(r_cnt > 0)
		{
			...
			long delta_time =  1000000l*(end1.tv_sec - begin.tv_sec) + (end1.tv_usec-begin.tv_usec);
			printf("recv from usb size:%d,end1 delta-time:%ld us\n", r_cnt, delta_time);
		    //Storing files is just for demonstration. 
		    //Users should use their own decoder instead of saving files.
		    //printf("write data to pipe r_cnt : %d\n", r_cnt);
		    ret = write(pipe_fd, buf0, r_cnt);
			#if 0
		    w_cnt = 0;
		    do{
		        //ret = send(clientSockfd, &buf0[w_cnt], 1024, 0);
		        //ret = RingBufferPut(&buf0[w_cnt], r_cnt - w_cnt);
		        ret = write(pipe_fd, &buf0[w_cnt], r_cnt - w_cnt);
		        if(ret > 0)
		            w_cnt += ret;
		        else if(ret < 0)
...
int main(int argc, char** argv) {
	...
	//printf("usb init ok, open fifo ok , pthread created ok\n");
  ...
  {
    char const* streamName = "live_h264";
